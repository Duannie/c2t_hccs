...
void main(int initround, monster foe, string page) {
	...
	if (my_familiar() == $familiar[Chocolate lab]) {
		c2t_bb($skill[gingerbread mob hit]).c2t_bbSubmit();
        return;	
	}	
	...
	//saber random thing at this location for meteor shower buff -- saber happens elsewhere
		...
		switch (foe) {
			...
			case $monster[mother slime]:
				c2t_bb($skill[snokebomb]).c2t_bbSubmit();
				return;	

			case $monster[amateur ninja]:
				c2t_bb($skill[asdon martin: missile launcher]).c2t_bbSubmit();
				return;		
